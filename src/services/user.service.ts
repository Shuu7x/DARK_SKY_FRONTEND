...
export interface IUserService {
  ...
  create(params: Omit<IUser, 'id'>): Promise<Omit<IUser, 'password'>>
  ...
  update(params: IUser): Promise<Omit<IUser, 'password'>>
}

export const userService: IUserService = {
  changePassword: function (params: Pick<IUser, 'id' | 'password'>): Promise<void> {
    const { id, ...body } = params
    return apiClient.post(`/users/${id}/password`, body)
  },
  create: function (params: Omit<IUser, 'id'>): Promise<Omit<IUser, 'password'>> {
  ...
  update: function (params: IUser): Promise<Omit<IUser, 'password'>> {
    const { id, ...body } = params
